...
def parse_RL_category(value):
    """Parses the 'RL Category' column (e.g., "RL1, RL4" or "RL1")."""
    if pd.isna(value) or not str(value).strip():
        return set()
    
    labels = set()
    # Split by comma, then process each part
    RL_items = str(value).split(',')
    for item in RL_items:
        item = item.strip() # Remove leading/trailing whitespace
        if item.startswith("RL"):
            try:
                # Extract digits after "RL"
                label_num = int(item[3:])
                labels.add(label_num)
            except ValueError:
                # Handle cases like "RL" without a number, or "RLabc"
                print(f"Warning: Could not parse number from RL item: '{item}' in '{value}'")
    return labels
